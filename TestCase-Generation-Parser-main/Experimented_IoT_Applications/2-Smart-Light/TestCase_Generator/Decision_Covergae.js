...
// ---------------- DRIVER CODE (ADD THIS AT THE BOTTOM) ----------------

const fs = require("fs");
const path = require("path");

// 1. Load Lambda files
const lambdaFolder = "Assets/lambdaFunctions";
const lambdaFiles = fs.readdirSync(lambdaFolder).filter(f => f.endsWith(".js"));

const lambdaList = lambdaFiles.map(file => {
    const code = fs.readFileSync(path.join(lambdaFolder, file), "utf-8");
    return {
...
// 2. Load TRD SQL rules (rule.json)
const trdList = JSON.parse(fs.readFileSync("Assets/rule.json", "utf-8")).map(r => ({
    ruleName: r.rule.ruleName,
    sql: r.rule.sql
}));

// 3. Load ESM (esm.json)
const esmList = JSON.parse(fs.readFileSync("Assets/esm.json", "utf-8"));

// 4. Load ERD (erd.json)
const erdList = JSON.parse(fs.readFileSync("Assets/erd.json", "utf-8"));

// 5. Generate Decision Coverage
const tableDC = generateDecisionCoverage(lambdaList, trdList, esmList, erdList);

// 6. Print final table
console.table(tableDC);
